...
namespace REST_API.Controllers
{
    public class NewSettingsController : ApiController
    {
        ...
        [Route("api/newsettings/delete/file/{token}/{SettingsID}")]
        public Response DeleteFile(string token, int SettingsID)
        {
            Token t = Token.Exists(token);
            if (t == null)
            {
        ...
        [Route("api/newsettings/delete/database/{token}/{SettingsID}")]
        public Response DeleteDatabase(string token, int SettingsID)
        {
            Token t = Token.Exists(token);
            if (t == null)
            {
            ...
            using (MySqlConnection connection = WebApiConfig.Connection())
            {
                try
                {
                    ...
                    string sql = "DELETE FROM daemonsSettingsDatabase WHERE id = @id; ";
                    MySqlCommand query = new MySqlCommand(sql, connection);
                    query.Parameters.AddWithValue("@id", SettingsID);
